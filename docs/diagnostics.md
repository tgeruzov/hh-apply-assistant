...
- удалить сохранённый log и metrics через **Очистить всё**;
...
`hh_apply_assistant_v4_metrics` содержит:
...
Metrics не ограничивают число разных counter names, но их набор задаётся исходником. Кнопка **Очистить всё** создаёт новый объект с `startedAt` и пустыми секциями.
...
Healthcheck проверяет шесть групп selectors с учётом текущей страницы:
...
Textarea или modal button вне открытой формы закономерно неприменимы. На пустой выдаче list selectors также не считаются ошибкой. В конце Healthcheck пишет summary и состояние instance lock.
...
2. Выполните Healthcheck на проблемной странице.
