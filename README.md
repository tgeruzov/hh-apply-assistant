...
* Вылезла капча (решите её руками и нажмите Старт заново)
...
Если нужно отправлять: кликните по виджету, чтобы открыть окно настроек, включите тумблер и напишите текст. Шаблон один на все вакансии, сохраняется в браузере и не сбрасывается при перезагрузке.
...
* **Страница зависла:** если скрипт больше 60 секунд не двигается дальше, сработает таймер зависания - он перезагрузит страницу (до двух раз) и попробует продолжить. Если не помогло, скрипт остановится с ошибкой.
* **Ошибка в виджете:** при сбое счётчик в виджете краснеет, а кнопка меняется на `Сброс`. Откройте виджет - там карточка с описанием. Нажмите `Понятно`, затем `Сброс` и `Старт`.
...
- Скрипт рассылает кастомные события `hha:status`, `hha:outcome`, `hha:progress` и т. п. через `window.dispatchEvent`.
