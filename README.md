...
Внизу страницы висит плавающий виджет с двумя цифрами:
* цифра у самолётика - отправлено откликов за сегодня. Счётчик считается по вашему времени, обнуляется в полночь и хранится в браузере. Лимит hh считает скользящие 24 часа, поэтому цифры могут не совпадать
* цифра у списка - вакансий в очереди на ручной отклик

Во время работы в пилюле рядом со счётчиком виден текущий шаг (`Пауза 4 с`, `Отклик`, `Жду hh.ru`), а полоска снизу заполняется, пока идёт пауза. Если навести на счётчик, появится подсказка с расшифровкой цифр и полным описанием шага. То же описание есть в строке статуса наверху раскрытой панели. Когда скрипт не работает, там сводка за день, а после остановки итог запуска: сколько времени он шёл и сколько откликов, вакансий в очередь и пропусков за это время. Причина остановки видна в подсказке при наведении на эту строку.

<p align="center">
  <img src="docs/images/pill-idle.png" width="226" alt="Виджет в простое: 37 откликов за сегодня, 4 в очереди и кнопка Старт">
  &nbsp;&nbsp;
  <img src="docs/images/pill-running.png" width="284" alt="Виджет во время работы: шаг Пауза 3 с и полоска паузы">
</p>
...
Откройте вкладку `Очередь` в окне виджета. Клик по строке с вакансией откроет её в новой вкладке - ответьте на вопросы и отправьте руками. Открытые вакансии помечаются как `Просмотрено` и уходят вниз списка. Пока вакансия лежит в очереди, скрипт не открывает её повторно, даже в новой вкладке.

Когда вы откликнулись, вакансия сама пропадает из очереди: скрипт видит подтверждение на её странице, в том числе если вы ответили на вопросы в окне без перезагрузки страницы. Такой отклик прибавляется к счётчику за сегодня, потому что hh.ru тоже учитывает его в суточном лимите. Крестик в строке убирает вакансию без счёта, например если вы решили не откликаться, а `Очистить всё` просит подтверждения. Очередь и счётчик общие для всех вкладок hh.ru и обновляются в них без перезагрузки.
...
* Вы уже откликались на неё раньше. Такие вакансии скрипт помнит 30 дней и больше не открывает, даже если hh показывает на карточке кнопку отклика
...
* **Подробный лог:** если ошибка плавающая или описания из виджета недостаточно - откройте виджет и нажмите `Скопировать лог` внизу панели. В буфер попадёт лог: последние 600 записей, они сохраняются между сессиями. Если браузер не дал доступ к буферу обмена, кнопка так и напишет, а лог выведется в консоль (<kbd>F12</kbd>, вкладка `Console`). Из консоли лог можно получить и напрямую: `copy(hhaDumpLog())`, очистить: `hhaClearLog()`.
...
- В глобальном объекте `window` есть `HHApplyAssistant` и функции `hhaDumpLog`, `hhaClearLog`. Объект `HHApplyAssistant` только для консоли, и в нём минимум: `start`, `stop`, `getState`, `getManualQueue`, `on`, `off` и те же функции лога. Настройки, сброс состояния и изменение очереди доступны только виджету изнутри скрипта, в том числе через элемент `<hha-hud>` до них не добраться.
...
3. Данные хранятся только у вас: настройки, текст письма, очередь, счётчик, список вакансий с откликом и лог лежат в `localStorage` вашего браузера.
4. Лог не пишет текст письма и персональные поля: фильтр `SENSITIVE_KEYS` отбрасывает ключи вроде `coverText`, `name`, `email`, `phone`. Зато в лог попадают технические данные: id вакансий, адреса страниц, коды ошибок и при сбоях фрагмент HTML страницы (до 160 символов). Когда скрипт пропускает вакансию или отправляет её в очередь по тексту страницы, в лог пишется сработавшее правило, фрагмент этого текста (тоже до 160 символов) и, если правило сработало по элементу, его селектор. Хранятся последние 600 записей.
